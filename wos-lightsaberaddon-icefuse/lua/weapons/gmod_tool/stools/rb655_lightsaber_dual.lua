...
File Path:   addons/wos-lightsaberaddon-icefuse/lua/weapons/gmod_tool/stools/rb655_lightsaber_dual.lua
...
function TOOL:Deploy()
	...
	for _, model in pairs( wOS.DefaultSaberModels ) do
		table.insert( tbl, model )
	end
	
	if wOS.SaberGroupModels[ group ] then
		for _, model in pairs( wOS.SaberGroupModels[ group ] ) do
			table.insert( tbl, model )
		end
	end
