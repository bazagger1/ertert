...
File Path:   addons/[weapons]_baton/lua/weapons/icefuse_baton/shared.lua
...
SWEP.Author = "Icefuse"
SWEP.Contact = ""
SWEP.Purpose = "Use to arrest your target or stun them"
SWEP.IconLetter = ""
...
SWEP.Category = "Icefuse Clone Weapons"
...
hook.Add("canDropWeapon", "NoDropPoliceBaton",function(ply, ent)
	if ent:GetClass() == "icefuse_baton" then
		return false;
	end
end)
...
if CLIENT then
	hook.Add("PlayerBindPress","test1", function(ply,bind,pressed)
		...
		if weap == nil or weap == NULL or not IsValid(weap) or weap:GetClass() !="icefuse_baton" then return end;
...
if SERVER then
	...
	net.Receive("batonsendfunc",function(leng,ply)
		...
		if ply:GetActiveWeapon():GetClass() != "icefuse_baton" then return end;
